...
pub fn action(register: Json<RegsiterRequest>, db: &State<DbPool>) -> Result<Json<RegisterResponse>, ApiError> {
    ...
    if let Some(_) = UserRepository::find_user_by_username(&mut conn, &register.username) {
        return Err(ApiError {
    ...
    if UserRepository::store(&mut conn, &register.username, &register.password, &register.name).is_ok() {
